...
class Post extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, content, category_id, user_id, likes, dislikes, privacy', 'required'),
			array('category_id, user_id, likes, dislikes', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>200),
			array('privacy', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, content, category_id, user_id, likes, dislikes, privacy', 'safe', 'on'=>'search'),
		);
	}
