'use strict';
myCustom.controller('userconnectsController', function userconnectsController($scope, $http, $window, $navigate, $routeParams) {
	...
	$scope.connectUsers = function() {
		$http({
				url: $scope.baseurl+"connectInUsers/"+$scope.uid,
				method : 'GET',
				params : {}
			}).success(function(data) {
				//alert(JSON.stringify(data));
